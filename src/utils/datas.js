 ...
 */
export function formatarDataExtenso(dataStr, opcoes = {}) {
  if (!dataStr) return '-';
  try {
 ...
 * Ex: '2026-07-28' → 'Ter'
 */
export function formatarDiaSemana(dataStr, formato = 'short') {
  if (!dataStr) return '-';
  try {
    const dataNorm = /^\d{4}-\d{2}-\d{2}$/.test(dataStr) ? dataStr + 'T12:00:00' : dataStr;
 ...
 */
export function formatarDataHora(timestampStr) {
 ...
 */
export function formatarHora(timestampStr) {
  if (!timestampStr) return '-';
  try {
