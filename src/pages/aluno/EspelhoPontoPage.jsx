...
import { Printer, Download, Calendar, Upload, FileText } from 'lucide-react';
...
export const EspelhoPontoPage = () => {
  ...
  // Modal de Justificativa / Anexo de Atestado
  ...
  const handleSubmeterJustificativa = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      showToast('Erro ao enviar justificativa: ' + (err.message || 'Tente novamente.'), 'erro');
    } finally {
      setEnviando(false);
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.download = `espelho-ponto-${usuario?.matricula || 'aluno'}-${mesAno}.csv`;
    link.click();
    URL.revokeObjectURL(url);
    showToast('Espelho de ponto baixado em formato CSV!', 'sucesso');
  };

  return (
    <section>
      {/* Cabeçalho de Ações na Tela */}
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 className="page-title">Espelho de Ponto Individual</h1>
      ...
      {/* DOCUMENTO OFICIAL DO ESPELHO DE PONTO */}
      <div className="printable-voucher" style={{ background: '#FFFFFF', padding: '2.5rem', borderRadius: '16px', border: '1px solid var(--border-color)', boxShadow: '0 4px 16px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2.5px solid var(--primary)', paddingBottom: '1.25rem', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            ...
            <h2 style={{ color: 'var(--primary)', fontSize: '1.25rem', fontWeight: 800, margin: 0 }}>
              CLÍNICA-ESCOLA UNINASSAU — ESPELHO DE PONTO INDIVIDUAL
            </h2>
          ...
          <div style={{ textAlign: 'right', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
            ...
            <div style={{ fontFamily: 'monospace', fontSize: '0.85rem', background: '#F1F5F9', padding: '3px 8px', borderRadius: 4, marginTop: 4 }}>
              NASSAU-PONTO-{usuario?.matricula || '0000'}-{mesAno}
            </div>
            ...
            <tbody>
              {registrosFiltrados.length === 0 ? (
                ...
                registrosFiltrados.map((r) => (
                  <tr key={r.id} style={{ backgroundColor: r.status_frequencia === 'ausencia' ? '#FEF2F2' : r.status_frequencia === 'atraso' ? '#FFFBEB' : 'transparent' }}>
                    ...
                    <td><strong style={{ color: !r.hora_saida ? '#991B1B' : 'var(--text-dark)' }}>{r.hora_saida || 'Em andamento'}</strong></td>
                    ...
                    <td>
                      ...
                      {r.status_frequencia !== 'atraso' && r.status_frequencia !== 'ausencia' && <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>-</span>}
                    </td>
                  </tr>
                ))
              )}
        ...
        {/* Rodapé de Assinaturas para Validação Oficial */}
      ...
      {/* ─── MODAL DINÂMICO DE JUSTIFICATIVA E ANEXO DE ATESTADO ─── */}
      {itemParaJustificar && (
        <div className="modal-overlay">
          <div className="modal-card" style={{ maxWidth: 540 }}>
            ...
            <form onSubmit={handleSubmeterJustificativa}>
              ...
              {/* Campo de texto de justificativa */}
