...
export const CalendarioVagasPage = ({ setActiveTab }) => {
  ...
  const fetchCalendario = useCallback(async () => {
    ...
    try {
      ...
      showToast('Erro ao carregar calendário: ' + err.message, 'erro');
      setDiasCalendario([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDiaClick = async (dia) => {
    ...
    setSlots([]);
    try {
      const diaSemanaJS = dia.diaSemana; // 0=Dom..6=Sab
      const data = await agendamentoService.getDisponibilidade('', diaSemanaJS, dia.data);
      setSlots(data.slots || []);
    } catch (err) {
      showToast('Erro ao carregar horários: ' + err.message, 'erro');
    } finally {
  ...
  const handleConfirmar = async () => {
    ...
    try {
      ...
      showToast('Erro ao registrar: ' + err.message, 'erro');
    } finally {
  ...
  return (
    <section>
      ...
      <div className={`calendar-layout-container ${diaAtivo ? 'has-active-day' : ''}`}>
        ...
        {/* PAINEL / MODAL DE HORARIOS */}
        {diaAtivo && (
          <div className="slots-modal-backdrop" onClick={(e) => { if (e.target.className === 'slots-modal-backdrop') setDiaAtivo(null); }}>
                      ...
                      <div key={slot.vaga_id} style={{ border: `2px solid ${cfgSlot.border}`, borderRadius: 12, padding: '0.85rem', background: cfgSlot.bg }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
                          ...
                          <span style={{ fontSize: '0.72rem', fontWeight: 700, color: cfgSlot.text, background: '#FFF', padding: '2px 8px', borderRadius: 20, border: `1px solid ${cfgSlot.border}` }}>
                            {disponivel ? `${slot.vagasDisponiveis} livre${slot.vagasDisponiveis !== 1 ? 's' : ''}` : 'LOTADO'}
                          </span>
                        </div>
                        ...
                        <div style={{ marginTop: '0.5rem', background: '#E2E8F0', borderRadius: 999, height: 5, overflow: 'hidden' }}>
                          <div style={{ width: `${slot.capacidade_max > 0 ? (slot.vagasOcupadas / slot.capacidade_max) * 100 : 0}%`, height: '100%', background: cfgSlot.border, borderRadius: 999 }} />
                        </div>
                        <p style={{ margin: '3px 0 0.6rem', fontSize: '0.7rem', color: 'var(--text-muted)' }}>
                          {slot.vagasOcupadas}/{slot.capacidade_max} ocupadas
                        </p>
